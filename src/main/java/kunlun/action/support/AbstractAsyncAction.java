/*
 * Copyright (c) 2018. the original author or authors.
 * Kunlun is licensed under the "LICENSE" file in the project's root directory.
 */

package kunlun.action.support;

import kunlun.core.handler.AsyncSupportedHandler;
import kunlun.util.Assert;

/**
 * The abstract asynchronous action.
 * @author Kahle
 */
public abstract class AbstractAsyncAction
        extends AbstractAutoAction implements AsyncSupportedHandler {

    public AbstractAsyncAction(String actionName) {

        super(actionName);
    }

    @Override
    public Object execute(String strategy, Object input, Object[] arguments) {
        // The input object cannot be null.
        Assert.notNull(input, "Parameter \"input\" must not null. ");
        // Build async supported context.
        AsyncSupportContext context = buildContext(input, null); // todo actionUtils
        // The default is synchronous call.
        if (context.getAsync() == null || !context.getAsync()) {
            // Synchronous call.
            return doExecute(context);
        }
        else {
            // Asynchronous call.
            AsyncExecuteTask task = new AsyncExecuteTask(this, context);
            context.setFuture(context.getThreadPool().submit(task));
            // Get the result.
            context.setFinish(false);
            return getResult(context);
        }
    }

}
